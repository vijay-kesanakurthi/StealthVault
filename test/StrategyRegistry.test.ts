...
async function registerDemoAndGetId(reg: StrategyRegistry, signer: HardhatEthersSigner): Promise<string> {
  const tx = await reg.connect(signer).registerStrategyDemo(DEMO_BUY, DEMO_SELL);
...
describe("StrategyRegistry", function () {
  ...
  it("registers and exposes FHE handles", async function () {
    const strategyId = await registerDemoAndGetId(registry, owner);
  ...
  describe("execution nonce + applyResult", function () {
    ...
    it("pays keeper only when nav>0 and action is BUY (not HOLD)", async function () {
      const strategyId = await registerDemoAndGetId(registry, owner);
      await registry.connect(owner).deposit(strategyId, { value: ethers.parseEther("1") });
    ...
    it("no keeper reward on HOLD even with nav", async function () {
      const strategyId = await registerDemoAndGetId(registry, owner);
      await registry.connect(owner).deposit(strategyId, { value: ethers.parseEther("1") });
      await owner.sendTransaction({ to: await registry.getAddress(), value: ethers.parseEther("0.1") });
      const nonce = await executeAndGetNonce(registry, strategyId, 90n, owner);
